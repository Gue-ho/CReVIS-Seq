import time, operator, os, sys, argparse, gzip
...
class CrevisSeq:
	...
	def DB_modeling(self):
		...
		for i in list(range(1, 23))+['X', 'Y']:
			exin_db[str(i)] = {}
		with gzip.open('./DB/exon_intron_v2.txt.gz') as f:
			for line in f:
				line_sp = line.decode("utf-8").strip().split()
				chro = line_sp[0]
				st = int(line_sp[7])
				ed = int(line_sp[8])
				a = st
	...
	def analyze_fastmap_results(self):
		...
		chromomap_dict = {}
		chromomap_id = 0
		with open(self.output_name+'.txt', 'w') as fw, open('{0}_chromomap.txt'.format(self.output_name[:self.output_name.rfind('.')]), 'w') as fwc:
			id_c = 0
			fw.write('ID\tsequence\tchromosome\tstart\tread\tgene\tclass\texon ID\n')
			fwc.write('ID\tsequence\tchromosome\tstart\tread\tgene\tclass\texon ID\n')
